 ...
 ******************************************************************************
  ...
  ******************************************************************************
  *
  * APP_TRHEAD State Machine API:
  * APP_THREAD_SendCoapMsg()
  *    --> Send Non-Confirmable Put Request. Called by EXTI Interrupt
  ...
  *                            NULL);
  *
  * --> Start Timer Single Shot Timeout to 5s
  *  --> Wait Timer Event after timeout
  *  --> Send Confirmable Put Request.
  *              APP_THREAD_CoapSendRequest(&OT_Ressource,
  ...
  *                            NULL);
  *
  * APP_THREAD_CoapSendRequest()
  *  --> Configure Coap message with specifed parameters and send it
  *  --> Send PayloadWrite[] buffer contains
  *
  *
  * APP_THREAD_CoapRequestHandler()
  *  --> Handler called when server receive a Coap Request
  *  --> Read pMessage and write it to PayloadRead[]
  *  --> If received message Mode is Confirmable, then send response
  *              APP_THREAD_CoapSendDataResponse()
  *
  *
  * APP_THREAD_CoapSendDataResponse()
  *  --> Function acknowledge back to sender with same message
  *
  * APP_THREAD_CoapDataRespHandler()
  *  --> Handler called to manage response
  *  --> Simply test and printf if NO_ERROR case
  *
  */
...
/* USER CODE BEGIN PD */
#define C_RESSOURCE             "adcconv"
#define COAP_PAYLOAD_LENGTH     7U
#define WAIT_TIMEOUT            (1 * 1000 * 1000 / CFG_TS_TICK_VAL) /**< 1s */
#ifndef OUTDOOR_DEVICE
#define LED_FLASH_TIME          (1000 * 1000 / CFG_TS_TICK_VAL / 5) /**< 0.2s */
#define VCP_BUFFER_SIZE         64U
#endif
/* USER CODE END PD */
...
static void APP_THREAD_InitPayloadRead(void);
static void APP_THREAD_SendCoapMsg(void);
static void APP_THREAD_TimingElapsed(void);
static void APP_THREAD_LedTimingElapsed(void);
...
static uint8_t TimerID, TimerLED;

#ifdef OUTDOOR_DEVICE
static uint32_t CoapSend = 0;
static uint32_t CoapAck = 0;
static float ackRatio = 0.0;
#endif
...
extern uint16_t adcIn1Raw;
extern uint16_t adcIn2Raw;
extern uint16_t adcIn3Raw;
...
void APP_THREAD_Init( void )
{
  ...
  /** RR registers CFG_TASK_COAP_MSG_BUTTON in the sequencer */
  ...
  /**> RR TimerID used for Coap Send Confirmable */
  HW_TS_Create(CFG_TIM_PROC_ID_ISR, &TimerID, hw_ts_SingleShot, APP_THREAD_TimingElapsed);
  /**> RR TimerID used for INDOOR Led Flashing */
  HW_TS_Create(CFG_TIM_PROC_ID_ISR, &TimerLED, hw_ts_SingleShot, APP_THREAD_LedTimingElapsed);

  /**> RR Initialize Payload */
  APP_THREAD_InitPayloadWrite();
  APP_THREAD_InitPayloadRead();
...
static void APP_THREAD_DeviceConfig(void)
{
  ...
  /**> RR Set OpenThread radio Tx power to 6dBm */
  error = otPlatRadioSetTransmitPower(NULL, 6);
  if (error != OT_ERROR_NONE)
  {
    APP_THREAD_Error(ERR_THREAD_SETUP, error);
  }
  else
  {
    APP_DBG("Radio Tx Power set to maximum")
  }
...
static void APP_THREAD_StateNotif(uint32_t NotifFlags, void *pContext)
{
  ...
  /*                                LED2(GREEN) LED3(RED)
   *    OT_DEVICE_ROLE_DISABLED     OFF         OFF
   *    OT_DEVICE_ROLE_DETACHED     OFF         OFF
   *    OT_DEVICE_ROLE_CHILD        OFF         ON
   *    OT_DEVICE_ROLE_ROUTER       OFF         ON
   *    OT_DEVICE_ROLE_LEADER       ON          OFF
   */
                                       ...
                                       void* aContext)
{
  ...
  do{
    ...
    {
#ifdef OUTDOOR_DEVICE
      /**> RR Coap message succefully send */
      CoapSend++;
#endif
  }
}
                                          ...
                                          const otMessageInfo * pMessageInfo)
{
  ...
  char vcpTxBuffer[VCP_BUFFER_SIZE];
  uint8_t buffLen;

  memset(vcpTxBuffer, 0, sizeof(vcpTxBuffer));

  /**> RR Serialize data in JSON format */
  buffLen = sprintf(vcpTxBuffer, "{\"adc_in0\": %d, \"adc_in1\": %d, \"adc_in2\": %d, \"ratio\": %d}\n\r",
                    PayloadRead[0] + (PayloadRead[1] << 8),
                    PayloadRead[2] + (PayloadRead[3] << 8),
                    PayloadRead[4] + (PayloadRead[5] << 8),
                    PayloadRead[6]);

  /**> RR Send it over UART */
  HAL_UART_Transmit_DMA(&huart1, (uint8_t *)vcpTxBuffer, buffLen);

  /**> Start Led Flashing timer 2x */
  BSP_LED_On(LED1);
  HW_TS_Start(TimerLED, (uint32_t)LED_FLASH_TIME);
  UTIL_SEQ_WaitEvt(LED_EVENT_TIMER);
  BSP_LED_Off(LED1);
#endif

  if (APP_THREAD_CheckMsgValidity() == true)
  {
    /**> RR to be implemented */
  }
...
#ifdef OUTDOOR_DEVICE
    /**> Coap data response ACK successfully received */
    CoapAck++;
    ackRatio = 100 * CoapAck / CoapSend;
#endif
  }
  else
  {
...
/**
 * @brief Initialize CoAP read buffer.
 * @param  None
 * @retval None
 */
static void APP_THREAD_InitPayloadRead(void)
...
{
#ifdef OUTDOOR_DEVICE
  /**> RR Update Payload field */
  PayloadWrite[0] = (uint8_t)(adcIn1Raw & 0xFF);
  PayloadWrite[1] = (uint8_t)(adcIn1Raw >> 8);

  PayloadWrite[2] = (uint8_t)(adcIn2Raw & 0xFF);
  PayloadWrite[3] = (uint8_t)(adcIn2Raw >> 8);

  PayloadWrite[4] = (uint8_t)(adcIn3Raw & 0xFF);
  PayloadWrite[5] = (uint8_t)(adcIn3Raw >> 8);

  PayloadWrite[6] = (uint8_t)(ackRatio);
...
static void APP_THREAD_LedTimingElapsed(void)
{
  UTIL_SEQ_SetEvt(LED_EVENT_TIMER);
}
